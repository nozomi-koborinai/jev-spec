...
describe('assertRubric', () => {
  it('enforces noul probability thresholds', () => {
    expect(assertRubric('r', noulRubric, noulResult(0.9), { minProbability: 0.85 }).passed).toBe(
      true
    );
    expect(assertRubric('r', noulRubric, noulResult(0.84), { minProbability: 0.85 }).passed).toBe(
      false
    );
    expect(assertRubric('r', noulRubric, noulResult(0.1), { maxProbability: 0.15 }).passed).toBe(
      true
    );
    expect(assertRubric('r', noulRubric, noulResult(0.16), { maxProbability: 0.15 }).passed).toBe(
      false
    );
  });
  ...
  it('fails a noul assertion when the evaluator returns a non-finite probability', () => {
    ...
    expect(
      assertRubric('r', noulRubric, noulResult(Number.NaN), { maxProbability: 0.15 }).passed
    ).toBe(false);
  });

  it('enforces allowed and blocked choices', () => {
    expect(
      assertRubric('r', choiceRubric, choiceResult('secure', 0.9), { allowedChoices: ['secure'] })
        .passed
    ).toBe(true);
    expect(
      assertRubric('r', choiceRubric, choiceResult('insecure', 0.9), { allowedChoices: ['secure'] })
        .passed
    ).toBe(false);
    expect(
      assertRubric('r', choiceRubric, choiceResult('insecure', 0.9), {
  ...
  it('fails a choice assertion when the confidence is below minConfidence or non-finite', () => {
    ...
    expect(assertRubric('r', choiceRubric, choiceResult('secure', 0.7), assertion).passed).toBe(
      false
    );
    expect(
      assertRubric('r', choiceRubric, choiceResult('secure', Number.NaN), assertion).passed
    ).toBe(false);
  });

  it('enforces score thresholds', () => {
    expect(assertRubric('r', scoreRubric, scoreResult(1.9, 0.9), { minScore: 1.8 }).passed).toBe(
      true
    );
    expect(assertRubric('r', scoreRubric, scoreResult(1.7, 0.9), { minScore: 1.8 }).passed).toBe(
      false
    );
    expect(assertRubric('r', scoreRubric, scoreResult(1.2, 0.9), { maxScore: 1.0 }).passed).toBe(
      false
    );
    expect(
      assertRubric('r', scoreRubric, scoreResult(Number.NaN, 0.9), { minScore: 1.8 }).passed
    ).toBe(false);
  });
  ...
  it('enforces minConfidence on score assertions', () => {
    ...
    expect(assertRubric('r', scoreRubric, scoreResult(1.5, Number.NaN), assertion).passed).toBe(
      false
    );
  });
