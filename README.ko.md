...
| :--- | :--- | :--- | :--- |
| **Bun** | `>= 1.2` (최신) | Tier 1 / 공식 지원 | pre-commit 훅, 로컬 개발 루프 |
| **Node.js** | `>= 22.0.0` (LTS 22) | Tier 1 / 공식 지원 | CI/CD 파이프라인, 컨테이너 환경 |
| **Node.js** | `>= 24.0.0` (Current 24) | Tier 1 / 공식 지원 | 현재 버전의 Node 런타임 환경 |
...
- **TypeScript 설정**: `jev-spec.config.ts`는 두 런타임 모두에서 직접 로드됩니다.
