...
- The user asks to set up, install, init or configure jev-spec, or to add zones or rubrics to an existing setup
...
Look for `jev-spec.config.ts`, `jev-spec.config.js` or `jev-spec.config.mjs` at the repo root. If one exists, ask: `(overwrite)`, `(add)` zones and rubrics for uncovered requirements, or `(skip)`. Never rewrite a tuned config silently: its thresholds may have been calibrated.

### 2. Install

If `jev-spec` is already a dependency, keep it and only check the version. Otherwise pick the package manager from the lockfile (`bun.lock` → bun, `pnpm-lock.yaml` → pnpm, `yarn.lock` → yarn, otherwise npm) and install `jev-spec` as a dev dependency. Then run `npx jev-spec --version` and `npx jev-spec --help`. The help output is the source of truth for flags; do not rely on memory.
...
3. Propose **one zone per specification file or domain**: `specPath` (a single file), `codePaths` (globs, tests excluded with `!**/*.test.ts`), and `specFilter.requirementPrefix` when one file holds several domains.
4. Show the mapping and let the user confirm it before writing anything.

Keep zones small. Everything in a zone is sent in one request (at most 500 files, 2 MB per file, about 120,000 characters of code), and the model's accuracy drops as unrelated content grows. A zone that covers the whole `src/` tree is a bug.
...
- **One `noul` per zone for drift**: `'Does the code add behavior that the specification does not describe, such as …?'` with two or three examples that fit the domain. Its threshold is a `maxProbability`. Like every threshold it goes into `assertions`: `noul()` takes the question and nothing else.
...
- **`choice` and `score` only when something depends on them.** A generic "security posture" or "completeness" rubric is a vague, zone-wide judgment. Add one when the user names a decision it drives.
...
**README section.** Offer a short "Spec verification" block with the two commands (`npx jev-spec check`, `npx jev-spec check --zone <name>`).
