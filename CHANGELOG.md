...
- **`--staged` and `--diff` choose the targets and no longer shrink what the model sees.** A target none of whose code files changed is still skipped. A touched target is now checked on all its files in full, as in a full run. Until now only the changed hunks were sent, and a rubric whose subject lay outside them failed: in a live run on this repository, staging a one-line comment change failed four rubrics (0.49 to 0.64) that pass at 0.92 to 0.97 on the whole files. A deleted file counts as a change, also when it was the last file of its target. A run with `--staged` reads the staged content from the git index, so that after a partial `git add` it judges what is about to be committed. The terminal report, the dry run and `targets[].changedFiles` in JSON say which files selected a target. `CodeExtractionOptions.contextLines` is gone, and `CodeFileContext.source` is always `'file'`.
- **Custom evaluators return `{ answers, model? }`.** `JevEvaluator.evaluate()` resolves to `{ answers, model? }` instead of the bare record of answers, so that an evaluator can report which model answered. `LiveJevEvaluator` accepts a client as a second constructor argument, which makes it testable without the network.
...
- New README section, "jev-spec checks itself": the project's own specs, probes and the recorded result (22 of 22 probes caught with `jev-1.13.0`; a full check of 8 targets and 22 rubrics takes under four seconds and an estimated $0.0006).
...
- The README's runtime section states measurements instead of claims. Gone are "< 100ms" and "350ms – 500ms" cold starts, "70ms – 400ms" for an evaluation, "3x faster", and the statement that start-up is most of the time a hook takes. Measured on the maintainer's laptop: the CLI starts in about 0.1 s on Node and 0.05 s on Bun, and a live check of one target takes 0.3 to 0.9 s, so the request decides. The section also says that `bunx jev-spec` runs on Node because of the shebang, and that `bunx --bun jev-spec` runs it on Bun.
...
- A pre-commit hook or a workflow that runs `--staged` or `--diff` needs no change. It now checks every touched target in full, which costs a little more per run and no longer fails rubrics whose subject lies outside the changed lines.
...
- jev-spec checks itself. `docs/specs/` holds 25 requirements (exit codes, failing closed, diff runs, path and git safety), and `jev-spec.config.ts` pairs 22 of them with the one to three files that implement each group: 8 targets and 22 rubrics, with three of the spec files serving two or three targets each through `specFilter.requirementPrefix`. The other three requirements are pinned by tests and listed in `docs/specs/checked-by-tests.md`, because no wording made the model check them: one follows a value through a function, one depends on what a regular expression accepts, and one means handing a function the list of changed files in place of the patterns. The test suite runs a dry run of that configuration and fails on any warning, on a requirement ID that is declared twice, and on a requirement that neither a rubric nor a test title names.
...
- The first live runs of jev-spec on itself are recorded in `docs/probe-results.md`: all 22 rubrics pass on the intact code and fail on the copy that their probe breaks (`jev-1.13.0`). Getting there changed the rubrics, not the thresholds: direct, literal questions about the mechanism, without the requirement ID, separate intact from broken code far better than "Does the code satisfy REQ-X: …?", which was answered "yes" for broken code in 4 of the first 20 cases; smaller targets help as well. The runs also found dead code: an unused error class in the check command could have exited with `1`; it is gone.
- `docs/specs/diff-runs.md` states the requirements of a diff run, with two small targets, rubrics and probes of their own. One of the probes is a bug that review found in the first version of the change: a target whose files were all deleted was skipped, because the skip looked at the files that were left instead of the files that had changed.
- CI checks jev-spec against its own specs with the live model on every pull request and on every push to `main` (`.github/workflows/jev-spec.yml`). The job reports in the run summary and does not block yet. Pull requests from forks have no API key and get a dry run instead. It is a full run on purpose: a diff run skips every target whose code did not change, also when its spec did.
...
- `CONTEXT.md` defines the project's vocabulary (check, target, drift, rubric, assertion, run and the words each of them replaces), and `AGENTS.md` points to it. The code and the documentation of this release are aligned with it.
