...
- `docs/specs/diff-runs.md` states the requirements of a diff run, with two small targets, rubrics and probes of their own: 22 of 22 probes caught. One of the probes is a bug that review found in the first version of this change: a target whose files were all deleted was skipped, because the skip looked at the files that were left instead of the files that had changed. A fifth requirement, that a touched target is read in full, is pinned by a test instead: no wording made the model see what it means to hand a function the list of changed files in place of the patterns.
- CI checks jev-spec against its own specs with the live model on every push and pull request (`.github/workflows/jev-spec.yml`). The job reports in the run summary and does not block yet. Pull requests from forks have no API key and get a dry run instead. It is a full run on purpose: a diff run sends only the changed hunks, and a one-line comment change was enough to fail four rubrics whose subject lay outside the hunk.
