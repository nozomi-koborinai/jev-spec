...
| :--- | :--- | :--- | :--- |
| **Bun** | `>= 1.2`（最新） | Tier 1 / フルサポート | pre-commit フック、ローカル開発ループ |
| **Node.js** | `>= 22.0.0`（LTS 22） | Tier 1 / フルサポート | CI/CD パイプライン、コンテナ環境 |
| **Node.js** | `>= 24.0.0`（Current 24） | Tier 1 / フルサポート | 現行の Node ランタイム環境 |
...
- **TypeScript の設定**: `jev-spec.config.ts` は、どちらのランタイムでも直接読み込まれます。
